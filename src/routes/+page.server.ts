...
export const load = (async () => {
	const tennisProduct = await productService.getPaginatedFilter({ page: 1, pageSize: 1 }, 2);
	const footProduct = await productService.getPaginatedFilter({ page: 1, pageSize: 1 }, 4);
	const runningProduct = await productService.getPaginatedFilter({ page: 1, pageSize: 1 }, 0);
	const newComers = [
		{
